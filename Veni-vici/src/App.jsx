...
function App() {
  ...
  const addSeenCat = (cat) => {
    setSeenCats((prevSeenCats) => {
      if (prevSeenCats.some((seenCat) => seenCat.reference_image_id === cat.reference_image_id)) {
        return prevSeenCats;
      }
      return [...prevSeenCats, cat];
    });
  };
      ...
      <div className="left-section">
        ...
        <div className="seen-cats-list">
          {seenCats.length === 0 ? (
            ...
            seenCats.map((seenCat) => (
              <div key={seenCat.reference_image_id} className="seen-cat-card">
                <h3>{seenCat.randomName}</h3>
                <p>{seenCat.name}</p>
                <img
                  className="seen-cat-image"
                  src={`https://cdn2.thecatapi.com/images/${seenCat.reference_image_id}.jpg`}
                  alt={seenCat.name}
                />
              </div>
            ))
